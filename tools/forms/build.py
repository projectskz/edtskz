...
def konstruktor_form():
    ...
    page_step = page("СтраницаШаг", "Шаг", [
        ...
        field("ЭлементШагаШаблонОповещения", "ЭлементШага.ШаблонОповещения", "Шаблон оповещения", events=ch,
              ToolTip="Тема и текст с параметрами ([Процесс], [Предмет.Номер], [Переменные.Сумма]…). Если шаблон "
                      "не выбран — тема: название шага, текст: поле ниже. Кому — вкладка «Оповещения»"),
        group("ГруппаТаймер", [
    ...
    page_notify = page("СтраницаОповещения", "Оповещения", [
        table("ОповещенияШага", "ОповещенияШага", cols("ОповещенияШага", [
            ("Событие", "Когда", "field", {"ListChoiceMode": True}),
            ("ВидПолучателя", "Кому", "field", {"events": on_change("ОповещенияШагаВидПолучателяПриИзменении")}),
            ("Получатель", "Кто именно", "field", {
                "ToolTip": "Роль, пользователь, путь к реквизиту предмета («Ответственный», «Переменные.Контролёр»), "
                           "адрес e-mail или обработчик — по виду получателя. Для исполнителя, автора и наблюдателей "
                           "не заполняется"}),
            ("Каналы", "Каналы", "field", {"InputHint": "как настроил получатель",
                                           "ToolTip": "Пусто — каналы из личных настроек получателя "
                                                      "(по умолчанию «в программе» и почта)"}),
            ("Шаблон", "Шаблон", "field", {"InputHint": "стандартный текст"}),
            ("Срочное", "Срочно", "check", {"ToolTip": "Отправить сразу, не собирать в сводку; получатель не может "
                                                       "отписаться"}),
        ]), events={"OnChange": "СвойствоТаблицыПриИзменении", "OnStartEdit": "ОповещенияШагаПриНачалеРедактирования"},
            Height=5, TitleLocation="None"),
        label("ПодсказкаОповещений", "ПодсказкаОповещений", TitleLocation="None", Height=2),
    ])
    ...
    page_none = page("СтраницаПусто", "Нет элемента", [
        deco("ПодсказкаПусто", "Выберите шаг или линию в списке выше или дважды щёлкните элемент на схеме. "
                               "На вкладке «Оповещения» — оповещения процесса целиком: о запуске, завершении "
                               "и о задачах всех шагов."),
    ])

    props = group("Свойства", [
        pages("СтраницыСвойств", [page_none, page_step, page_exec, page_buttons, page_fields, page_notify,
                                    page_line],
    ...
    attrs = [
        ...
        Attr("ОповещенияШага", T_VT, "Оповещения", columns=[
            ("Событие", t_cfg("EnumRef.кбп_СобытияПроцессов"), "Когда"),
            ("ВидПолучателя", t_cfg("EnumRef.кбп_ВидыПолучателейОповещений"), "Кому"),
            ("Получатель", t_cfg("CatalogRef.кбп_РолиИсполнителей") + REF_USER
             + t_cfg("CatalogRef.кбп_ОбработчикиПроцессов") + t_str(255), "Кто именно"),
            ("Каналы", t_str(255), "Каналы"),
            ("Шаблон", t_cfg("CatalogRef.кбп_ШаблоныСообщений"), "Шаблон"),
            ("Срочное", T_BOOL, "Срочно"),
        ]),
        Attr("ПодсказкаОповещений", str0, "Подсказка"),
...
def shablon_form():
    """Форма шаблона сообщения (ФТ-603): тема и текст с параметрами, тексты для отдельных каналов."""
    items = [
        field("Наименование", "Объект.Description", "Название", InputHint="Например: Новая задача (кратко)"),
        field("Событие", "Объект.Событие", "Для события",
              ToolTip="Шаблон события используется для стандартных оповещений, если в схеме шаблон не выбран"),
        field("Тема", "Объект.Тема", "Тема", InputHint="Новая задача: [Задача]"),
        field("Текст", "Объект.Текст", "Текст", TitleLocation="Top", MultiLine=True, Height=6),
        check("ЭтоHTML", "Объект.ЭтоHTML", "Текст в формате HTML (для почты)"),
        label("ПараметрыШаблона", "ПараметрыШаблона", "Параметры", TitleLocation="Top", Height=3),
        table("ТекстыПоКаналам", "Объект.ТекстыПоКаналам", cols("ТекстыПоКаналам", [
            ("Канал", "Канал", "field", {"ListChoiceMode": True}),
            ("Тема", "Тема", "field", {"InputHint": "как выше"}),
            ("Текст", "Текст", "field", {"InputHint": "как выше"}),
            ("ЭтоHTML", "HTML", "check"),
        ], path="Объект.ТекстыПоКаналам"), Height=3, Title="Другой текст для канала", TitleLocation="Top"),
    ]
    attrs = [
        Attr("Объект", t_cfg("CatalogObject.кбп_ШаблоныСообщений"), main=True, saved=True),
        Attr("ПараметрыШаблона", t_str(0), "Параметры"),
    ]
    return form(items, attrs, [], events={"OnCreateAtServer": "ПриСозданииНаСервере"})
...
def moi_zadachi_form():
    commands = [
        ...
        Cmd("ПрочитатьВсе", "Прочитать все", "Отметить все уведомления прочитанными"),
        Cmd("НастройкиОповещений", "Настройки оповещений…",
            "Какие оповещения получать, по каким каналам и собирать ли письма в сводку"),
    ]
    notices = table("Уведомления", "Уведомления", cols("Уведомления", [
        ("Дата", "Когда", "field", {"Width": 12}),
        ("Текст", "Уведомление", "field"),
    ]), events={"Selection": "УведомленияВыбор"}, bar=[button("УведомленияПрочитатьВсе", "ПрочитатьВсе"),
                                                     button("УведомленияНастройкиОповещений", "НастройкиОповещений")],
        ReadOnly=True, ChangeRowSet=False, ChangeRowOrder=False, TitleLocation="None")
    ...
    items = [pages("Страницы", [
        ...
        page("СтраницаУведомления", "Уведомления", [notices], TitleDataPath="ЗаголовокУведомлений"),
    ], PagesRepresentation="TabsOnTop")]
    attrs = [
        Attr("Объект", t_cfg(f"DataProcessorObject.{MOI}"), main=True),
        ...
        Attr("Уведомления", T_VT, "Уведомления", columns=[
            ("Ид", t_str(36), "Ид"),
            ("Дата", T_DATETIME, "Когда"),
            ("Текст", t_str(0), "Уведомление"),
            ("Задача", t_cfg("CatalogRef.кбп_Задачи"), "Задача"),
            ("Процесс", t_cfg("CatalogRef.кбп_Процессы"), "Процесс"),
            ("Прочитано", T_BOOL, "Прочитано"),
        ]),
        Attr("ПоказыватьВыполненные", T_BOOL, "Показывать выполненные"),
        Attr("ЗаголовокЗадачи", t_str(0), "Заголовок страницы задач"),
        Attr("ЗаголовокУведомлений", t_str(0), "Заголовок страницы уведомлений"),
    ]
    bar = [button("Форма" + c.name, c.name, usual=False) for c in commands
           if c.name not in ("ПрочитатьВсе", "НастройкиОповещений")]
    return form(items, attrs, commands, bar=bar, bar_autofill=False, Title="Мои задачи",
                events={"OnOpen": "ПриОткрытии", "OnCreateAtServer": "ПриСозданииНаСервере",
                        "NotificationProcessing": "ОбработкаОповещения"})


def nastroiki_opoveshenii_form():
    commands = [Cmd("ЗаписатьИЗакрыть", "Записать и закрыть", "Записать настройки оповещений")]
    items = [
        group("ГруппаОбщие", [
            field("Каналы", "Каналы", "Каналы по умолчанию", InputHint="в программе и почта",
                  ToolTip="Куда присылать оповещения, если в схеме процесса канал не указан"),
            group("ГруппаГруппировка", [
                field("СпособГруппировки", "СпособГруппировки", "Письма присылать",
                      events=on_change("СпособГруппировкиПриИзменении"),
                      ToolTip="Сразу — каждое оповещение отдельным письмом; иначе несрочные письма собираются "
                              "в одну сводку"),
                field("ИнтервалМинут", "ИнтервалМинут", "каждые, мин", Width=5, HorizontalStretch=False),
            ], horizontal=True),
        ], title="Общие", show_title=False),
        table("События", "События", cols("События", [
            ("Событие", "Событие", "field", {"ReadOnly": True}),
            ("Получать", "Получать", "check"),
            ("Каналы", "Каналы", "field", {"InputHint": "по умолчанию"}),
        ]), bar=[], ChangeRowSet=False, ChangeRowOrder=False, Height=8, TitleLocation="None"),
        deco("ПодсказкаНастроек", "Оповещения, которые автор схемы отметил как срочные, приходят всегда и сразу. "
                                  "Канал «В программе» — уведомления на вкладке «Уведомления» в «Моих задачах»."),
    ]
    attrs = [
        Attr("Объект", t_cfg(f"DataProcessorObject.{MOI}"), main=True),
        Attr("Каналы", t_str(255), "Каналы по умолчанию"),
        Attr("СпособГруппировки", t_cfg("EnumRef.кбп_СпособыГруппировкиОповещений"), "Письма присылать"),
        Attr("ИнтервалМинут", t_num(5, 0, True), "Интервал, минут"),
        Attr("События", T_VT, "События", columns=[
            ("Событие", t_cfg("EnumRef.кбп_СобытияПроцессов"), "Событие"),
            ("Получать", T_BOOL, "Получать"),
            ("Каналы", t_str(255), "Каналы"),
        ]),
    ]
    bar = [button("ФормаЗаписатьИЗакрыть", "ЗаписатьИЗакрыть", usual=False, DefaultButton=True)]
    return form(items, attrs, commands, bar=bar, Title="Настройки оповещений",
                events={"OnCreateAtServer": "ПриСозданииНаСервере"})
...
def process_form():
    commands = [
        ...
        Cmd("ПриостановитьПроцесс", "Приостановить", "Приостановить процесс: задачи нельзя выполнить, таймеры ждут"),
        Cmd("ПродолжитьПроцесс", "Продолжить", "Продолжить приостановленный процесс"),
    ...
    bar = [button("ФормаПрерватьПроцесс", "ПрерватьПроцесс", usual=False),
           button("ФормаПриостановитьПроцесс", "ПриостановитьПроцесс", usual=False),
           button("ФормаПродолжитьПроцесс", "ПродолжитьПроцесс", usual=False),
...
# --------------------------------------------------------------------------------------------------------------------
# Панель администратора (ФТ-804, ФТ-901, ФТ-904, ФТ-905)
# --------------------------------------------------------------------------------------------------------------------

PANEL = "кбп_ПанельАдминистратора"


def panel_form():
    commands = [
        Cmd("Обновить", "Обновить", "Перечитать списки", "StdPicture.Refresh"),
        Cmd("ОткрытьПроцесс", "Открыть процесс", "Открыть карточку выбранного процесса", "StdPicture.Change"),
        Cmd("ПриостановитьПроцесс", "Приостановить", "Приостановить процесс: задачи нельзя выполнить, таймеры ждут"),
        Cmd("ПродолжитьПроцесс", "Продолжить", "Продолжить приостановленный процесс или процесс с ошибкой; "
                                         "действия с ошибкой выполняются заново"),
        Cmd("ПерезапуститьШаг", "Перезапустить шаг", "Отменить задачи выбранного шага и войти в него заново"),
        Cmd("ПеревестиНаШаг", "Перевести на шаг", "Перевести выбранное положение процесса на шаг из поля "
                                                  "«Перевести на шаг»"),
        Cmd("ПрерватьПроцесс", "Прервать процесс", "Прервать процесс и отменить его задачи"),
        Cmd("ПовторитьДействие", "Повторить", "Выполнить действие заново"),
        Cmd("ОтменитьДействие", "Отменить", "Больше не выполнять действие"),
        Cmd("ПовторитьОтправку", "Отправить заново", "Отправить выделенные сообщения заново"),
        Cmd("ОтменитьОтправку", "Не отправлять", "Отменить отправку выделенных сообщений"),
        Cmd("ОтправитьСейчас", "Отправить очередь сейчас", "Не ждать регламентного задания"),
        Cmd("ОтправитьРассылки", "Отправить рассылки сейчас",
            "Записать настройки и сразу разослать включённые сводки (для проверки текста)"),
        Cmd("ЗаписатьНастройки", "Записать настройки", "Записать настройки продукта", "StdPicture.Write"),
        Cmd("ВключитьЗадания", "Включить регламентные задания",
            "Включить обработку очередей, контроль сроков, рассылки и очистку журнала"),
    ]
    processes = table("Процессы", "Процессы", cols("Процессы", [
        ("Ссылка", "Процесс", "field"),
        ("Состояние", "Состояние", "field", {"Width": 12}),
        ("Проблема", "Что не так", "field"),
        ("Автор", "Автор", "field", {"Width": 15}),
        ("ДатаСтарта", "Запущен", "field", {"Width": 12}),
    ]), events={"Selection": "ПроцессыВыбор", "OnActivateRow": "ПроцессыПриАктивизацииСтроки"},
        bar=[button("ПроцессыОбновить", "Обновить"), button("ПроцессыОткрытьПроцесс", "ОткрытьПроцесс")],
        ReadOnly=True, ChangeRowSet=False, ChangeRowOrder=False, TitleLocation="None", Height=8)
    markers = table("Маркеры", "Маркеры", cols("Маркеры", [
        ("Шаг", "Где сейчас", "field"),
        ("Состояние", "Состояние", "field", {"Width": 10}),
        ("Исполнители", "У кого", "field"),
        ("ДатаСоздания", "С", "field", {"Width": 12}),
    ]), bar=[], ReadOnly=True, ChangeRowSet=False, ChangeRowOrder=False, TitleLocation="None", Height=3)
    actions = group("ГруппаВыбранный", [
        markers,
        group("ГруппаПеревод", [
            field("НовыйШаг", "НовыйШаг", "Перевести на шаг", ListChoiceMode=True, Width=30,
                  ToolTip="Шаг той же версии схемы. Задачи текущего положения отменяются, вход в шаг выполняется "
                          "как при обычном переходе"),
            field("Комментарий", "Комментарий", "Комментарий", InputHint="обязателен: попадёт в историю процесса"),
        ], horizontal=True),
        command_bar("ДействияПроцесса", [
            button("ДействияПриостановить", "ПриостановитьПроцесс", usual=False),
            button("ДействияПродолжить", "ПродолжитьПроцесс", usual=False),
            button("ДействияПерезапуститьШаг", "ПерезапуститьШаг", usual=False),
            button("ДействияПеревестиНаШаг", "ПеревестиНаШаг", usual=False),
            button("ДействияПрерватьПроцесс", "ПрерватьПроцесс", usual=False),
        ]),
    ], title="Выбранный процесс", show_title=True, representation="NormalSeparation", TitleDataPath="ЗаголовокВыбранного")
    errors = table("ОшибкиДействий", "ОшибкиДействий", cols("ОшибкиДействий", [
        ("Вид", "Действие", "field", {"Width": 14}),
        ("Процесс", "Процесс", "field"),
        ("ПоследняяОшибка", "Ошибка", "field"),
        ("Попыток", "Попыток", "field", {"Width": 5}),
        ("Статус", "Статус", "field", {"Width": 8}),
        ("ВыполнитьПосле", "Когда", "field", {"Width": 12}),
    ]), events={"Selection": "ОшибкиДействийВыбор"},
        bar=[button("ОшибкиДействийПовторить", "ПовторитьДействие"), button("ОшибкиДействийОтменить", "ОтменитьДействие")],
        ReadOnly=True, ChangeRowSet=False, ChangeRowOrder=False, TitleLocation="None")
    queue = table("Оповещения", "Оповещения", cols("Оповещения", [
        ("ДатаСоздания", "Создано", "field", {"Width": 12}),
        ("Канал", "Канал", "field", {"Width": 8}),
        ("Кому", "Кому", "field", {"Width": 15}),
        ("Тема", "Тема", "field"),
        ("Статус", "Статус", "field", {"Width": 10}),
        ("Попыток", "Попыток", "field", {"Width": 5}),
        ("Ошибка", "Ошибка", "field"),
    ]), events={"Selection": "ОповещенияВыбор"},
        bar=[button("ОповещенияПовторить", "ПовторитьОтправку"), button("ОповещенияОтменить", "ОтменитьОтправку"),
             button("ОповещенияОтправитьСейчас", "ОтправитьСейчас")],
        ReadOnly=True, ChangeRowSet=False, ChangeRowOrder=False, TitleLocation="None", SelectionMode="MultiRow")
    num = dict(Width=6, HorizontalStretch=False)
    settings = [
        group("ГруппаНастройкиПроцессов", [
            field("МаксимальнаяГлубинаВложенности", "МаксимальнаяГлубинаВложенности", "Предел вложенности процессов",
                  ToolTip="Защита от бесконечной рекурсии вложенных процессов (по умолчанию 20)", **num),
            field("СрокХраненияЖурналаДней", "СрокХраненияЖурналаДней", "Хранить историю завершённых процессов, дней",
                  ToolTip="0 — хранить всегда. Очищает регламентное задание «Очистка журнала»", **num),
            field("АдминистраторПроцессов", "АдминистраторПроцессов", "Администратор процессов",
                  ToolTip="Получает задачи, для которых не нашёлся исполнитель, и эскалации без роли"),
        ], title="Процессы", show_title=True, representation="NormalSeparation"),
        group("ГруппаНастройкиВремени", [
            field("ПроизводственныйКалендарь", "ПроизводственныйКалендарь", "Производственный календарь",
                  ListChoiceMode=True, Width=30, HorizontalStretch=False,
                  ToolTip="Для сроков в рабочих днях и часах. Пусто — основной календарь программы"),
            group("ГруппаРабочийДень", [
                field("НачалоРабочегоДня", "НачалоРабочегоДня", "Рабочий день с, ч", **num),
                field("ОкончаниеРабочегоДня", "ОкончаниеРабочегоДня", "до, ч", **num),
            ], horizontal=True),
        ], title="Рабочее время", show_title=True, representation="NormalSeparation"),
        group("ГруппаНастройкиОповещений", [
            field("КаналыПоУмолчанию", "КаналыПоУмолчанию", "Каналы по умолчанию", Width=30, HorizontalStretch=False,
                  ToolTip="Если ни в схеме, ни в личных настройках пользователя канал не выбран. "
                          "Например: Интерфейс, Почта"),
            check("ОтправлятьПочту", "ОтправлятьПочту", "Отправлять электронную почту",
                  ToolTip="Выключите, пока не настроена системная учётная запись почты"),
        ], title="Оповещения", show_title=True, representation="NormalSeparation"),
        group("ГруппаРассылки", [
            group("ГруппаСводка", [
                check("СводкаЗадач", "СводкаЗадач", "Утренняя сводка «Ваши задачи на сегодня»",
                      events=on_change("РассылкиПриИзменении")),
                field("ЧасСводки", "ЧасСводки", "в, ч", **num),
            ], horizontal=True),
            group("ГруппаОтчетОПросрочках", [
                check("ОтчетОПросрочках", "ОтчетОПросрочках", "Отчёт о просроченных задачах руководителям",
                      events=on_change("РассылкиПриИзменении")),
                field("ЧасОтчета", "ЧасОтчета", "в, ч", **num),
            ], horizontal=True),
            field("РольПолучателейОтчета", "РольПолучателейОтчета", "Полный отчёт о просрочках также получает роль",
                  ToolTip="Например, «Контролёр процессов». Пусто — только руководители исполнителей"),
            check("РассылкиПоРабочимДням", "РассылкиПоРабочимДням", "Только в рабочие дни"),
            button("ОтправитьРассылкиКнопка", "ОтправитьРассылки"),
        ], title="Рассылки по расписанию", show_title=True, representation="NormalSeparation"),
        group("ГруппаЗадания", [
            label("СостояниеЗаданий", "СостояниеЗаданий", TitleLocation="None"),
            button("ВключитьЗаданияКнопка", "ВключитьЗадания"),
        ], title="Регламентные задания", show_title=True, representation="NormalSeparation"),
        button("ЗаписатьНастройкиКнопка", "ЗаписатьНастройки"),
    ]
    items = [pages("Страницы", [
        page("СтраницаПроцессы", "Процессы", [
            group("ГруппаОтбор", [
                field("ВидСписка", "ВидСписка", "Показать", ListChoiceMode=True, Width=25, HorizontalStretch=False,
                      events=on_change("ОтборПриИзменении")),
                field("ОтборСхема", "ОтборСхема", "Схема", events=on_change("ОтборПриИзменении")),
            ], horizontal=True),
            processes,
            actions,
        ], TitleDataPath="ЗаголовокПроцессов"),
        page("СтраницаОшибки", "Ошибки действий", [
            check("ПоказыватьОжидающие", "ПоказыватьОжидающие", "Показывать и ожидающие повтора",
                  events=on_change("ОтборПриИзменении")),
            errors,
            deco("ПодсказкаОшибок", "Действие получает статус «Ошибка» после 5 неудачных попыток, его процесс "
                                    "останавливается. «Повторить» выполняет действие заново и продолжает процесс."),
        ], TitleDataPath="ЗаголовокОшибок"),
        page("СтраницаОповещения", "Очередь оповещений", [
            field("ОтборОповещений", "ОтборОповещений", "Показать", ListChoiceMode=True, Width=25,
                  HorizontalStretch=False, events=on_change("ОтборПриИзменении")),
            queue,
        ], TitleDataPath="ЗаголовокОповещений"),
        page("СтраницаНастройки", "Настройки", settings),
    ], PagesRepresentation="TabsOnTop")]
    attrs = [
        Attr("Объект", t_cfg(f"DataProcessorObject.{PANEL}"), main=True),
        Attr("ВидСписка", t_str(50), "Показать"),
        Attr("ОтборСхема", t_cfg("CatalogRef.кбп_СхемыПроцессов"), "Схема"),
        Attr("Процессы", T_VT, "Процессы", columns=[
            ("Ссылка", t_cfg("CatalogRef.кбп_Процессы"), "Процесс"),
            ("Состояние", t_cfg("EnumRef.кбп_СостоянияПроцессов"), "Состояние"),
            ("Проблема", t_str(0), "Что не так"),
            ("Автор", REF_USER, "Автор"),
            ("ДатаСтарта", T_DATETIME, "Запущен"),
        ]),
        Attr("Маркеры", T_VT, "Где сейчас", columns=[
            ("ИдМаркера", t_str(36), "Маркер"),
            ("Шаг", t_cfg("CatalogRef.кбп_ЭлементыСхем"), "Где сейчас"),
            ("Состояние", t_cfg("EnumRef.кбп_СостоянияМаркеров"), "Состояние"),
            ("Исполнители", t_str(0), "У кого"),
            ("ДатаСоздания", T_DATETIME, "С"),
        ]),
        Attr("ТекущийПроцесс", t_cfg("CatalogRef.кбп_Процессы"), "Выбранный процесс"),
        Attr("НовыйШаг", t_cfg("CatalogRef.кбп_ЭлементыСхем"), "Перевести на шаг"),
        Attr("Комментарий", t_str(0), "Комментарий"),
        Attr("ОшибкиДействий", T_VT, "Ошибки действий", columns=[
            ("Ид", t_str(36), "Ид"),
            ("Вид", t_cfg("EnumRef.кбп_ВидыОтложенныхДействий"), "Действие"),
            ("Процесс", t_cfg("CatalogRef.кбп_Процессы"), "Процесс"),
            ("ПоследняяОшибка", t_str(0), "Ошибка"),
            ("Попыток", t_num(3, 0, True), "Попыток"),
            ("Статус", t_cfg("EnumRef.кбп_СтатусыОчереди"), "Статус"),
            ("ВыполнитьПосле", T_DATETIME, "Когда"),
        ]),
        Attr("ПоказыватьОжидающие", T_BOOL, "Показывать ожидающие"),
        Attr("ОтборОповещений", t_str(50), "Показать"),
        Attr("Оповещения", T_VT, "Очередь оповещений", columns=[
            ("Ид", t_str(36), "Ид"),
            ("ДатаСоздания", T_DATETIME, "Создано"),
            ("Канал", t_str(50), "Канал"),
            ("Кому", t_str(0), "Кому"),
            ("Тема", t_str(0), "Тема"),
            ("Статус", t_cfg("EnumRef.кбп_СтатусыОчереди"), "Статус"),
            ("Попыток", t_num(3, 0, True), "Попыток"),
            ("Ошибка", t_str(0), "Ошибка"),
            ("Процесс", t_cfg("CatalogRef.кбп_Процессы"), "Процесс"),
            ("Задача", t_cfg("CatalogRef.кбп_Задачи"), "Задача"),
        ]),
        Attr("МаксимальнаяГлубинаВложенности", t_num(3, 0, True), "Предел вложенности"),
        Attr("СрокХраненияЖурналаДней", t_num(5, 0, True), "Срок хранения истории, дней"),
        Attr("АдминистраторПроцессов", REF_USER, "Администратор процессов"),
        Attr("ПроизводственныйКалендарь", T_ANYREF, "Производственный календарь"),
        Attr("НачалоРабочегоДня", t_num(2, 0, True), "Начало рабочего дня"),
        Attr("ОкончаниеРабочегоДня", t_num(2, 0, True), "Окончание рабочего дня"),
        Attr("КаналыПоУмолчанию", t_str(255), "Каналы по умолчанию"),
        Attr("ОтправлятьПочту", T_BOOL, "Отправлять почту"),
        Attr("СводкаЗадач", T_BOOL, "Сводка задач"),
        Attr("ЧасСводки", t_num(2, 0, True), "Час сводки"),
        Attr("ОтчетОПросрочках", T_BOOL, "Отчёт о просрочках"),
        Attr("ЧасОтчета", t_num(2, 0, True), "Час отчёта"),
        Attr("РольПолучателейОтчета", t_cfg("CatalogRef.кбп_РолиИсполнителей"), "Роль получателей отчёта"),
        Attr("РассылкиПоРабочимДням", T_BOOL, "Только в рабочие дни"),
        Attr("СостояниеЗаданий", t_str(0), "Регламентные задания"),
        Attr("ЗаголовокПроцессов", t_str(0), "Заголовок страницы процессов"),
        Attr("ЗаголовокОшибок", t_str(0), "Заголовок страницы ошибок"),
        Attr("ЗаголовокОповещений", t_str(0), "Заголовок страницы оповещений"),
        Attr("ЗаголовокВыбранного", t_str(0), "Заголовок выбранного процесса"),
    ]
    return form(items, attrs, commands, bar=[], bar_autofill=False, Title="Панель администратора процессов",
                events={"OnCreateAtServer": "ПриСозданииНаСервере"})
...
FORMS = {
    ...
    ("DataProcessors/кбп_МоиЗадачи", f"DataProcessor.{MOI}", "НастройкиОповещений", "Настройки оповещений"):
        nastroiki_opoveshenii_form,
    ("Catalogs/кбп_Задачи", "Catalog.кбп_Задачи", "ФормаЭлемента", "Задача"): zadacha_form,
    ("Catalogs/кбп_ШаблоныСообщений", "Catalog.кбп_ШаблоныСообщений", "ФормаЭлемента", "Шаблон сообщения"):
        shablon_form,
    ("Catalogs/кбп_Процессы", "Catalog.кбп_Процессы", "ФормаЭлемента", "Процесс"): process_form,
    (f"DataProcessors/{PANEL}", f"DataProcessor.{PANEL}", "Форма", "Панель администратора процессов"): panel_form,
}
...
def grant_rights(role, obj, rights):
    """Права на объект в роли (перед первым объектом), например «Use», «View» для обработки."""
    path = SRC / "Roles" / role / "Ext" / "Rights.xml"
    text = path.read_bytes().decode("utf-8-sig")
    if f"<name>{obj}</name>" in text:
        return
    nl = "\r\n" if "\r\n" in text else "\n"
    block = f"<object>{nl}\t\t<name>{obj}</name>{nl}"
    for right in rights:
        block += f"\t\t<right>{nl}\t\t\t<name>{right}</name>{nl}\t\t\t<value>true</value>{nl}\t\t</right>{nl}"
    block += f"\t</object>{nl}\t"
    idx = text.index("<object>")
    text = text[:idx] + block + text[idx:]
    path.write_bytes(("\ufeff" + text).encode("utf-8"))
...
def add_to_subsystem(obj, subsystem="кбп_БизнесПроцессы"):
    path = SRC / "Subsystems" / f"{subsystem}.xml"
    text = path.read_bytes().decode("utf-8-sig")
    item = f'<xr:Item xsi:type="xr:MDObjectRef">{obj}</xr:Item>'
    if item in text:
        return
    nl = "\r\n" if "\r\n" in text else "\n"
    idx = text.index("</Content>")
    text = text[:idx] + f"\t{item}{nl}\t\t\t" + text[idx:]
    path.write_bytes(("\ufeff" + text).encode("utf-8"))


def write_stage4_metadata():
    write(f"DataProcessors/{PANEL}.xml", data_processor_md(PANEL, "Панель администратора процессов", ["Форма"], "Форма"))
    register_in_configuration("DataProcessor", PANEL, "DataProcessor")
    grant_rights("кбп_АдминистрированиеПроцессов", f"DataProcessor.{PANEL}", ["Use", "View"])
    add_to_subsystem(f"DataProcessor.{PANEL}")
...
def main():
    ...
    register_catalog_form("кбп_ШаблоныСообщений", "ФормаЭлемента")
