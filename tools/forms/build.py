...
def process_form():
    commands = [
        Cmd("Прервать", "Прервать процесс", "Прервать процесс и отменить его задачи"),
        Cmd("ДобавитьКомментарий", "Добавить комментарий", "Добавить комментарий в ленту процесса"),
    ...
    bar = [button("ФормаПрервать", "Прервать", usual=False), button("ФормаОбновить", "Обновить", usual=False)]
    return form(items, attrs, commands, bar=bar, bar_autofill=False,
                events={"OnCreateAtServer": "ПриСозданииНаСервере"})
